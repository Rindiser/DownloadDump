...
const createViewCoremaFields = () => {
    console.log('createViewCoremaFields')
...
async function db_delete(db,tableName) {
    return new Promise (function(resolve, reject) {
        if (tableName) {
...
function deleteFromTableSync (db, tableName, fileName, callbackFillTable) {
    // return new Promise (function(resolve, reject) {
        try {
            console.log('deleteTable ' + tableName)
            db.run(`DELETE FROM ${tableName};`, function(err) {
                ...
                console.log(`Row(s) deleted in ${tableName}: ${this.changes}`)
            })
            // resolve('success')
            callbackFillTable(db, tableName,fileName)
        } catch (error) {
            console.log(error)
            // reject(new Error(error))
        }
            ...
            console.log(filename)
                ...
                .on('data', (row) => {
                    ...
                    if (!filename.includes('corema')) {
                        ...
                        db.prepare( `INSERT INTO amplification (coreid, geneticAccessionURI, geneticAccessionNumber, BOLDProcessID) VALUES (?,?,?,?)`).run(row.coreid, row.geneticAccessionURI, row.geneticAccessionNumber, row.BOLDProcessID, function(err) {
                            if (err) {
                                console.log('error')
                                return console.error(err.message)
                            }
...
// function openDB(dataBaseFile, collection) {
    // return new Promise (function(resolve, reject) {
        // try {
            console.log(dataBaseFile + ' , ' + collection)
        ...
        if (i == rows.length-1) { // we reached the end of the file
            ...
            if (rows[i].organismID == rows[i+1].organismID) { // next line is item of same organism
                if (existingRow == 'nothing') { // first of two or more items
                    ...
                    if (rows[i].organismID === "urn:uuid:b270705b-ebf3-5cce-b4ec-87acf863ddeb") {
                    }
...
async function runCoremaStitch(collection, coremaFile, coremaFolder, outfile) {
    return new Promise (function(resolve, reject) { // with this, await is not allowed in this function (??)
        // try {
            ...
            dbPromise.then(function(db) {// with this, await is not allowed in this function
                ...
                //     } 
                // }
                // db.serialize(() => { // hvis vi bruker denne tror jeg ikke databasen endres. koden kjører på tomgang.
                // 1. delete data from tables in a sqlite-database (one per organismgroup), and fill tables again from musit- and coremadumpfiles
                
                changeEncoding(`${pathToCoremaDumps}${coremaFolder}/amplification.txt`)
                changeEncoding(`${pathToCoremaDumps}${coremaFolder}/materialsample.txt`)
                changeEncoding(`${pathToCoremaDumps}${coremaFolder}/multimedia.txt`)
                changeEncoding(`${pathToCoremaDumps}${coremaFolder}/permit.txt`)
                changeEncoding(`${pathToCoremaDumps}${coremaFolder}/preparation.txt`)
                changeEncoding(`${pathToCoremaDumps}${coremaFolder}/preservation.txt`)
                changeEncoding(`${pathToCoremaDumps}${coremaFolder}/resourcerelationship.txt`)
                changeEncoding(`${pathToCoremaDumps}${coremaFolder}/simpledwc.txt`)
                ...
                deleteFromTable(db,'amplification')
                deleteFromTable(db,'materialsample')
                deleteFromTable(db,'multimedia')
                deleteFromTable(db,'permit')
                deleteFromTable(db,'preparation')
                deleteFromTable(db,'preservation')
                deleteFromTable(db,'resourcerelationship')
                deleteFromTable(db,'simpledwc')
                ...
                db.run("BEGIN TRANSACTION"); // denne gjør at databasen lukker seg, kan ikke åpne den. den gjør kanskje også at tabellene ikke fylles. hvis jeg tar den bort går fillTable uendelig sakte.
                fillTable(db,'amplification',`${pathToCoremaDumps}${coremaFolder}/amplification.txt`)
                fillTable(db,'materialsample',`${pathToCoremaDumps}${coremaFolder}/materialsample.txt`)
                fillTable(db,'multimedia',`${pathToCoremaDumps}${coremaFolder}/multimedia.txt`)
                fillTable(db,'permit',`${pathToCoremaDumps}${coremaFolder}/permit.txt`)
                fillTable(db,'preparation',`${pathToCoremaDumps}${coremaFolder}/preparation.txt`)
                fillTable(db,'preservation',`${pathToCoremaDumps}${coremaFolder}/preservation.txt`)
                fillTable(db,'resourcerelationship',`${pathToCoremaDumps}${coremaFolder}/resourcerelationship.txt`)
                fillTable(db,'simpledwc',`${pathToCoremaDumps}${coremaFolder}/simpledwc.txt`)
                ...
                db.serialize(() => { // å ta bort denne fører oss ikke videre, inn i først db.run
                    ...
                    console.log('videre')
                    console.log(db)
                    // koden stopper her. har vi ikke kontakt med database?
                    ...
                    // .run(coremaSelect, (err, rows) => {
                        console.log('coremstitch here')
                        if (err) {
                            console.error(err.message)
                        }
...
async function runMusitCoremaStitch(collection, musitFile, coremaFolder, outfile, basedOn) {
    ...
    // const dbPromise = createDatabase(dataBaseFile, "musit", musitFile, collection)
    // dbPromise.then(function(db) {
console.log('linje 971 ' +  db)
    ...
    await changeEncoding(`${pathToCoremaDumps}${coremaFolder}/resourcerelationship.txt`)
    await changeEncoding(`${pathToCoremaDumps}${coremaFolder}/simpledwc.txt`) 
    // changeEncoding(`${pathToMusitDumps}${musitFile}/${musitFile}.txt`)
    //  changeEncoding(`${pathToCoremaDumps}${coremaFolder}/amplification.txt`)
    //  changeEncoding(`${pathToCoremaDumps}${coremaFolder}/materialsample.txt`)
    //  changeEncoding(`${pathToCoremaDumps}${coremaFolder}/multimedia.txt`)
    //  changeEncoding(`${pathToCoremaDumps}${coremaFolder}/permit.txt`)
    ...
    // ( without transaction, without await, does not work, must probably change the async function...)
    // (with transaction, without await, does not work, tables are not filled)
    // (with transactions, and with await does not work now; database is locked)
    // db.run("BEGIN TRANSACTION");
    ...
    await fillTable(db,'resourcerelationship',`${pathToCoremaDumps}${coremaFolder}/resourcerelationship.txt`)
    await fillTable(db,'simpledwc',`${pathToCoremaDumps}${coremaFolder}/simpledwc.txt`) 
    // fillTable(db, `${musitFile}`,`${pathToMusitDumps}${musitFile}/${musitFile}.txt`)
    //  fillTable(db, 'amplification',`${pathToCoremaDumps}${coremaFolder}/amplification.txt` )
    //  fillTable(db,'materialsample',`${pathToCoremaDumps}${coremaFolder}/materialsample.txt`)
    //  fillTable(db,'multimedia',`${pathToCoremaDumps}${coremaFolder}/multimedia.txt`)
    //  fillTable(db,'permit',`${pathToCoremaDumps}${coremaFolder}/permit.txt`)
    ...
    if (double === "no") {
        ...
        console.log('vi skulle kommet hit')
...
// sort sqlite-table, to see if we need to add more
async function sortTable () {
    const latestModified = 'SELECT MAX(modified) FROM catalogNumber'
...
//'SELECT MAX(modified) FROM simpledwc'
async function findMax (collection, musitFile, coremaFolder, outfile, basedOn) {
    let dataBaseFile = `${pathToDatabases}${collection}.db`
    console.log(dataBaseFile)
    let db = await createDatabase(dataBaseFile, "musit", musitFile, collection)
    // let db = new sqlite3.Database(dataBaseFile)
    console.log('linje 1170 ' + db)
    db.all('SELECT MAX(modified) FROM simpledwc', (err, rows) => {
...
async function mainSQLiteFunction () {
    ...
    await findMax('fish_herptiles','no_file')
    // await runCoremaStitch('birds2', 'no_file', 'NHMO-BI','birds_stitched.txt')
